...
<head>
    ...
    $(document).ready(function(){
        ...
        $('#menu_form').form('load', <?php echo $menu; ?>);
    });
    
    function submitForm(){
        if(!$('#menu_form').form('validate')){return false;}
        
        $.post('menuedit', $("#menu_form").serialize(), function(res){
            if(res.status>0){
                $.messager.alert('提示信息', '保存失败', 'error');
            }else{
                $.messager.alert('提示信息', '保存成功', 'info');
                parent.grid_reflesh();
                parent.dialog_close('dialog_edit');
            }
        });
    }
    </script>
...
<div class="easyui-panel" style="width:auto;">
    ...
    <form id="menu_form" method="post" action="menuedit">
        {!! csrf_field() !!}
